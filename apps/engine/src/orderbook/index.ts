...
export class Orderbook {
  ...
  public place_bid(order: Order) {
    ...
    const insert_bid_at = this.find_insert_position(
      "BID",
      Number.parseFloat(order.p),
    );
  ...
  public place_ask(order: Order) {
    ...
    const insert_ask_at = this.find_insert_position(
      "ASK",
      Number.parseFloat(order.p),
    );
  ...
  public process_order(side: Side, order: OrderRequest) {
    ...
    if (side === "ASK") {
      ...
      for (let i = 0; i < cbid_len; i++) {
        ...
        if (order_quan <= 0 || cbid_p < order_price_float) break;
        ...
        for (let j = i; j < this.bids.length; j++) {
          ...
          if (order_p < cbid_p || order_quan <= 0) break;
          else if (order_p > cbid_p) continue;
          ...
          Engine.get_instance().stream_depth(
            this.market,
            this.market_price,
            "BID",
          );
      ...
      this.bids = this.bids.filter((bid) => Number.parseFloat(bid.q) > 0.0);
      ...
      this.asks = this.asks.filter((ask) => Number.parseFloat(ask.q) > 0.0);
  ...
  public get_depth() {
    ...
    const asks = cumulative_asks.map(
      (ask) => [ask.price, ask.quantity.toFixed(2)] as const,
    );
    const bids = cumulative_bids.map(
      (bid) => [bid.price, bid.quantity.toFixed(2)] as const,
    );
    return {
      asks: asks,
      bids: bids,
  ...
  public lock_orderbook() {
    if (this.is_locked === true) throw new Error("Orderbook is already locked");
  ...
  private find_insert_position(side: "BID" | "ASK", order_price: number) {
    ...
    try {
      while (low <= high) {
        ...
        if (price == order_price) return mid;
