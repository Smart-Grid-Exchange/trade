...
export class Engine {
  ...
  private constructor() {
    ...
    if (parsed.success) {
      ...
      const mapped = MARKETS.map((market) => {
        ...
        return [
          market,
          new Orderbook(market, base_asset, quote_asset, [], [], "0.00", -1),
        ] as const;
      });
  ...
  public process_order(
    ...
    const order_id = this.assign_order_id(
      user_id,
      orderbook.market,
    ...
    try {
      const processed_order = orderbook.process_order(side, {
        q: order.q,
        p: order.p,
        id: order_id,
        ca: order.ca,
      ...
      RedisManager.get_instance().publish_to_api(
        order.client_id.toString(),
        JSON.stringify(publisher_payload),
      );
  ...
  public cancel_order(
    ...
    try {
      ...
      if (possibly_deleted_order !== undefined) {
        this.stream_depth(
          market,
          possibly_deleted_order.price,
          possibly_deleted_order.side,
        );
      }
    } catch (err) {
      console.log(err);
  ...
  public get_open_order(market: string, id: string, client_id: number) {
    ...
    const order_status =
      possible_order.eq !== "0.00"
        ? possible_order.q !== "0.00"
          ? "PART_FILLED"
          : "FILLED"
        : "NEW";
  ...
  public stream_depth(symbol: string, price: string, side: Side) {
    ...
    RedisManager.get_instance().publish_to_event_queue(
      `depth@${symbol}`,
      JSON.stringify({
        stream: `depth@${symbol}`,
        data: stream_payload,
