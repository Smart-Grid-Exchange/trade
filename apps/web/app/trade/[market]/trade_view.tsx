...
export default function TradeView({market}:{market: string}){
        ...
        async function fetch_klines(){
            ...
            try{
                const query_params = new URLSearchParams({
                    market: market,
                    start_time: (new Date().getTime() - 1000 * 60 * 60 * 24 * 7).toString(),
                    end_time: new Date().getTime().toString(),
                    interval: "1m"
                });

                const resp = await fetch(`http://localhost:3001/api/v1/klines?`+query_params,{
                    credentials: "include",
                    headers: {
    ...
    return (
        <div ref={chart_ref} style={{ height: "540px", width: "100%", marginTop: 4, borderRadius: 10 }}></div>
    )
}