    ...
    return(
        <Tabs defaultValue="bid" className="w-[370px] bg-slate-100 rounded-md h-full">
            ...
            <TabsContent value="bid">
                <OrderTypeTabBox side="BUY" symbol={symbol} market_price={market_price} />
            </TabsContent>
            <TabsContent value="ask">
                <OrderTypeTabBox side="SELL" symbol={symbol} market_price={market_price} />
            </TabsContent>
        </Tabs>
    )
}
...
function OrderTypeTabBox({side,market_price,symbol}:{side: "BUY" | "SELL", market_price: string, symbol: string}){
    return(
        <Tabs defaultValue="limit" className="w-[370px] my-1">
            ...
            <TabsContent value="limit"><LimitOrderBox side={side} symbol={symbol} market_price={market_price} /></TabsContent>
            <TabsContent value="market">
                <MarketOrderBox side={side} market_price={market_price} symbol={symbol} />
            </TabsContent>
        </Tabs>
    )
}
...
function MarketOrderBox({side, market_price, symbol}:{side: "BUY" | "SELL", market_price: string, symbol: string}){
    ...
    const {control, handleSubmit,getValues,watch, setValue, formState: {isDirty, isSubmitting, isLoading}} = form;
                                ...
                                <div className="">
                                    <Label className="flex justify-between my-1">
                                        <div className="flex gap-2">
                                            ...
                                            <ArrowLeftRight size={14}   onClick={() => setChoice("q")}/>
                                        </div>
                            ...
                            render={({field}) => (
                                <div>
                                    <Label className="flex justify-between my-1">
                                        <div className="flex gap-2">
                                            <span>Quantity</span>
                                            <ArrowLeftRight size={14} onClick={() => setChoice("p")}/>
                                        </div>
                                        <div>≈{(market_value * Number.parseFloat(quantity)).toFixed(2)} INR</div>
                                    </Label>
                                    <Input
                                    type="text"
                                    {...field}
...
function LimitOrderBox({
    ...
    const {control, handleSubmit,getValues, setValue, watch, formState:{isDirty, isSubmitting, isLoading}} = form;
